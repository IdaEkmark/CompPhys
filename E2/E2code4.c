...
void calculateEnergy(int n_timesteps, int n_particles, double **Q, double **P,
        double **kinE, double **potE, double **totE, int skipsize) 
{
    double wSq;
	for (int t = 0; t < n_timesteps/skipsize + 1; t++) { 
		for (int p = 0; p < n_particles; p++) {
            wSq = 4 * sin( (p+1)*PI /  ((double) 2*(n_particles + 1)) )*sin( (p+1)*PI / ((double) 2*(n_particles + 1)) );
			kinE[t][p] = 0.5 * P[skipsize*t][p]*P[skipsize*t][p];
            potE[t][p] = 0.5 * wSq * Q[skipsize*t][p]*Q[skipsize*t][p];
...
int main()
{
	double t_max = 1e6; int n_t = 10000000; double dt = t_max / (double) n_t; 
	int skipsize = 1000; int n_p = N_PARTICLES; double alpha = 0.1;
	...
	*/
    ...
    kinE = malloc((n_t/skipsize + 1) * sizeof *kinE);
	for (int i=0; i < n_t/skipsize + 1; i++){
		kinE[i] = malloc(n_p * sizeof *kinE[i]);
	}
	potE = malloc((n_t/skipsize + 1) * sizeof *potE);
	for (int i=0; i < n_t/skipsize + 1; i++){
		potE[i] = malloc(n_p * sizeof *potE[i]);
	}
	totE = malloc((n_t/skipsize + 1) * sizeof *totE);
	for (int i=0; i < n_t/skipsize + 1; i++){
		totE[i] = malloc(n_p * sizeof *totE[i]);
	}
	
	double *time_array;
	time_array = malloc((n_t/skipsize + 1) * sizeof (double));
	arange(time_array, 0, n_t/skipsize + 1, skipsize*dt);

    calculateEnergy(n_t, N_PARTICLES, Q, P, kinE, potE, totE, skipsize);
    saveKPEtoFile("4/KPE_1e-1.csv", time_array, kinE, potE, totE, n_t/skipsize, N_PARTICLES);
