...
int main()
{
    int N = 250; double dt = 0.05;
    double a = 1; double f1 = 2; double f2 = 6; double phi = 0;
    double time_array[N];
    ...
    double signal2[N];
    double signal[N];
    generate_signal(signal1, time_array, N, a, f1, phi);
    generate_signal(signal2, time_array, N, a, f2, phi);
    for(int i = 0; i < 250; i++){
    	signal[i] = signal1[i] + signal2[i];
	}
    write_to_file("signal26.csv", time_array, signal, N);
    return 0;
}
